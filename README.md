...
- **桌面宠物**：矢量卡通精灵浮窗随状态变化，**每个状态有多种随机动作**（空闲=呼吸/睡觉/翻滚，攻击=水枪/泡泡等，等待=招手/跳跃，完成=欢呼/撒花）；**右键 →「隐藏宠物」**，之后从菜单栏「显示宠物」重开；可拖动。三只原创精灵（草/火/水）× 四阶段（蛋/幼年/成熟/完全），每次安装随机分到一只（卸载重装重掷）。图鉴/动画预览见 [`docs/pet-preview.html`](./docs/pet-preview.html)。
- **能量/进化**：见下方「能量玩法」。
