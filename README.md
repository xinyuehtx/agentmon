...
- **桌面宠物**：随状态播放**原创手绘图集动画**（idle/工作/等待/完成，逐帧透明精灵）；**右键 →「隐藏宠物」**，之后从菜单栏「显示宠物」重开；可拖动。三只原创精灵（草/火/水）× 四阶段（蛋/幼年/成熟/完全），每次安装随机分到一只（卸载重装重掷）。图鉴见 [`docs/pet-sprites.png`](./docs/pet-sprites.png)。
  - 图集缺失时自动回退到内置**矢量卡通**渲染（预览 [`docs/pet-preview.html`](./docs/pet-preview.html)）。
  - 接新素材：按 [`docs/pet-art-prompt.md`](./docs/pet-art-prompt.md) 生成 6 帧横条 → `swift scripts/process-packs.swift <源目录>` 切片抠图 → `assets/pets_raster/`。
- **能量/进化**：见下方「能量玩法」。
